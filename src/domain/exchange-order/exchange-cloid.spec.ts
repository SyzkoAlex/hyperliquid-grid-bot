...
describe('ExchangeCloid', () => {
    describe('create', () => {
        it('should create exchange order uid from OrderId', () => {
            const orderId = OrderId.from('550d0a20-2bac-446a-8a2a-dbb1d378f564');
        ...
        it('should remove all dashes from UUID', () => {
            const orderId = OrderId.from('123e4567-e89b-41d4-a156-426614174000');
            const cloid = ExchangeCloid.create(orderId);
    ...
    describe('parse', () => {
        it('should parse valid cloid with 0x prefix to OrderId', () => {
        ...
        it('should parse valid cloid without 0x prefix to OrderId', () => {
        ...
        it('should round-trip correctly (create then parse)', () => {
            const originalOrderId = OrderId.from('550d0a20-2bac-446a-8a2a-dbb1d378f564');
            const cloid = ExchangeCloid.create(originalOrderId);
            const parsedOrderId = ExchangeCloid.parse(cloid.toString());

            expect(parsedOrderId).toBeInstanceOf(OrderId);
            expect(parsedOrderId?.toString()).toBe(originalOrderId.toString());
        });
    });
});
