...
describe('ExchangeCloid', () => {
    describe('create', () => {
        it('should create exchange order uid from order ID string', () => {
            const orderId = '550d0a20-2bac-446a-8a2a-dbb1d378f564';
        ...
        it('should remove all dashes from UUID', () => {
            const orderId = '123e4567-e89b-41d4-a156-426614174000';
            const cloid = ExchangeCloid.create(orderId);
    ...
    describe('parse', () => {
        it('should parse valid cloid with 0x prefix to UUID string', () => {
            const cloidStr = '0x550d0a202bac446a8a2adbb1d378f564';
            const result = ExchangeCloid.parse(cloidStr);

            expect(result).toBe('550d0a20-2bac-446a-8a2a-dbb1d378f564');
        });

        it('should parse valid cloid without 0x prefix to UUID string', () => {
        ...
        it('should handle different valid UUID formats', () => {
            ...
            expect(result).toBe('123e4567-e89b-41d4-a156-426614174000');
        });
        ...
        it('should round-trip correctly (create then parse)', () => {
            const originalOrderId = '550d0a20-2bac-446a-8a2a-dbb1d378f564';
            const cloid = ExchangeCloid.create(originalOrderId);
            const parsedOrderId = ExchangeCloid.parse(cloid.toString());

            expect(parsedOrderId).toBe(originalOrderId);
        });
    });
});
